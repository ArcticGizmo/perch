...
internal sealed class PreviewPane : Border
{
    ...
    /// <summary>
    /// Spotlights the overlay element a setting drives, so a user editing that setting can see which glyph
    /// it governs. A stub for now — the M4 shell wires it to a pulse on the affected region; kept here so
    /// the surrounding UI can already call it.
    /// </summary>
    public void Highlight(PreviewTarget target)
    {
        // TODO(M4): translate `target` to a screen region on the canvas and pulse it.
        _ = target;
    }
}
